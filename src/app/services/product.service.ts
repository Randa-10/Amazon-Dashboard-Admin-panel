...
export class ProductService {
  ...
  getProducts(): Observable<Products[]> {
    return this.http.get<Products[]>(
      `${environment.BaseApiURL}/products/adminProducts`,
      this.httpHeader
    );
  }
  getProductByID(prodID: string): Observable<Products> {
    return this.http.get<Products>(
      `${environment.BaseApiURL}/products/admin/${prodID}`
    );
  }
  updateProduct(prodID: string, product: Products): Observable<Products> {
    ...
    return this.http
      .patch<Products>(
        `${environment.BaseApiURL}/products/${prodID}`,
  ...
  SaveNewProduct(prd: Products): Observable<Products> {
    return this.http
      .post<Products>(
        `${environment.BaseApiURL}/products/addbyAdmin`,
        JSON.stringify(prd),
        this.httpHeader
      )
      .pipe(
  ...
  deletProduct(proID: string): Observable<Products> {
    return this.http.delete<Products>(
      `${environment.BaseApiURL}/products/${proID}`,
      this.httpHeader
    );
  }
  getNumberOfProducts(): Observable<number> {
    ...
    return this.http
      .get<{ count: number }>(endpoint, this.httpHeader)
